...
class Scan3RDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        object_attrs = torch.cat([torch.from_numpy(src_data_dict['object_attrs']), torch.from_numpy(ref_data_dict['object_attrs'])])

        ######## hyperedges
        src_hyperedges =  src_data_dict['hyperedges']
        ref_hyperedges =  ref_data_dict['hyperedges']
        hyperedges = torch.cat([torch.from_numpy(src_hyperedges).T, torch.from_numpy(ref_hyperedges).T])

        src_hyperedge_attrs =  src_data_dict['hyperedge_attrs']
        ref_hyperedge_attrs =  ref_data_dict['hyperedge_attrs']
